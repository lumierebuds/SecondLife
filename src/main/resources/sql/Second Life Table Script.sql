...
-- 거래글 사진 테이블 오리진 네임 속성 수정.
--ALTER TABLE BOARD_IMG MODIFY ORIGIN_NAME VARCHAR2(100);
...
ALTER TABLE CHAT_ROOM_JOIN ADD CONSTRAINT PK_CHAT_ROOM_JOIN_CRN_MN PRIMARY KEY (CHAT_ROOM_NO, MEMBER_NO);
...
CREATE TABLE CHAT_ROOM_JOIN (
    ...
    CONSTRAINT PK_CHAT_ROOM_JOIN_CRN_MN PRIMARY KEY (CHAT_ROOM_NO, MEMBER_NO),
    CONSTRAINT FK_CHAT_ROOM_JOIN_CRN FOREIGN KEY (CHAT_ROOM_NO)
        REFERENCES CHAT_ROOM (CHAT_ROOM_NO),
    CONSTRAINT FK_CHAT_ROOM_JOIN_MN FOREIGN KEY (MEMBER_NO)
        REFERENCES MEMBER (MEMBER_NO)
);
